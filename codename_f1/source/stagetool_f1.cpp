...
ZAY_VIEW_API OnGesture(GestureType type, sint32 x, sint32 y)
{
    ...
    else
    {
        if(type == GT_Pressed)
        {
            ...
            // 그룹 생성
            else if(m->mCurMonsterGroup != -1)
            {
                auto& CurMonsterGroup = m->mMonsterGroups[m->mCurMonsterGroup];
                sint32 CurEvent = m->mCurEvent;
                for(sint32 i = 0, iend = CurMonsterGroup.mIDs.Count(); i < iend; ++i)
                {
                    if(0 < i)
                    {
                        CurEvent += CurMonsterGroup.mDelaySecs[i - 1];
                        if(m->mWaves.At(m->mCurWave).mEventMonsters.Count() <= CurEvent)
                            break;
                    }
                    auto& NewMonster = m->mWaves.At(m->mCurWave).mEventMonsters.At(CurEvent).AtAdding();
                    NewMonster.mType = &m->mState.mMonsterTypes[CurMonsterGroup.mIDs[i]];
                    NewMonster.mRID = ++m->mState.mMonsterLastRID;
                    NewMonster.mPos.x = Pos.x;
                    NewMonster.mPos.y = Pos.y;
                }
    ...
    mMonsterGroupScroll(updater()), mMonsterGroupScrollMax(7)
{
    ...
    if(MonsterGroupTable.LoadJson(SO_NeedCopy, String::FromAsset("f1/table/monstergroup_table.json")))
    {
        ...
        {
            auto& NewMonsterGroup = mMonsterGroups.AtAdding();
            NewMonsterGroup.mName = MonsterGroupTable[i]("Name").GetString("");
            chars Script = MonsterGroupTable[i]("Group").GetString("");
            String Collector;
            bool IsTurnForID = true;
            do
            {
                if(*Script != '/' && *Script != '\0')
                    Collector += *Script;
                else if(0 < Collector.Length())
                {
                    if(IsTurnForID)
                    {
                        sint32 Index = 0;
                        ...
                        {
                            if(!mState.mMonsterTypes[i].mID.Compare(Collector))
                            {
                                Index = i;
                                break;
                            }
                        }
                        NewMonsterGroup.mIDs.AtAdding() = Index;
                    }
                    else NewMonsterGroup.mDelaySecs.AtAdding() = Parser::GetInt(Collector);
                    Collector.Empty();
                    IsTurnForID ^= true;
                }
            }
...
void stagetoolData::Load(chars filename)
{
    ...
    if(TextFile)
    {
        ...
        mBuildNumber = JsonWave("BuildNumber").GetInt(0);
...
void stagetoolData::Save(chars filename)
{
    ...
    {
        Context JsonWave;
        JsonWave.At("ToolType").Set("stagetool_f1");
        JsonWave.At("ToolVersion").Set(String::FromInteger(ToolVersion));
        JsonWave.At("BuildNumber").Set(String::FromInteger(mBuildNumber + 1));
...
void stagetoolData::Render(ZayPanel& panel)
{
    ...
    ZAY_FONT(panel, 1.0, "Arial Black")
    {
        ...
        {
            // 몬스터리스트
            if(const sint32 MonsterCount = mState.mMonsterTypes.Count())
                ...
                for(sint32 i = 0; i < MonsterGroupCount; ++i)
                {
                    ZAY_XYWH_UI(panel, 0, IconSize * (i - ScrollPos), panel.w(), IconSize, String::Format("monstergroup-%d", i),
                        ZAY_GESTURE_NT(n, t, this)
                        {
                            if(t == GT_InReleased)
                            {
                                mCurMonsterGroup = Parser::GetInt(&n[13]);
                                mCurMonster = -1;
                                const sint32 ScrollLimit = Math::Max(0, mMonsterGroups.Count() - mMonsterGroupScrollMax);
                                mMonsterGroupScroll.MoveTo(Math::Clamp(mCurMonsterGroup - mMonsterGroupScrollMax / 2, 0, ScrollLimit), 0.5);
                            }
                        })
                    {
                        ZAY_RGBA_IF(panel, 128, 255, 64, 192, i == mCurMonsterGroup)
                        ZAY_RGBA_IF(panel, 128, 128, 128, 192, i != mCurMonsterGroup)
                            panel.fill();

                        ZAY_RGB(panel, 0, 0, 0)
                            panel.rect(2);
                        ZAY_RGB(panel, 0, 0, 0)
                            panel.text(mMonsterGroups[i].mName, UIFA_CenterMiddle, UIFE_Right);
                    }
                }
                ZAY_INNER(panel, 2)
                ZAY_RGB(panel, 0, 0, 0)
