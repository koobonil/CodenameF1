...
namespace ZAY
{
        ...
        const Vector3& rkAxis)
    {
        ...
        float fSin = std::sin(fHalfAngle.valueRadians());
        w = std::cos(fHalfAngle.valueRadians());
        x = fSin*rkAxis.x;
        y = fSin*rkAxis.y;
        z = fSin*rkAxis.z;
    }
    ...
    void Quaternion::ToAngleAxis (Radian& rfAngle, Vector3& rkAxis) const
    {
        ...
        if ( fSqrLength > 0.0 )
        {
            ...
            float fInvLength = 1.0f / std::sqrt(fSqrLength);
    ...
    Quaternion Quaternion::Exp () const
    {
        ...
        Radian fAngle ( std::sqrt(x*x+y*y+z*z) );
        float fSin = std::sin(fAngle.valueRadians());

        Quaternion kResult;
        kResult.w = std::cos(fAngle.valueRadians());

        if ( std::fabs(fSin) >= msEpsilon )
        {
        ...
        const Quaternion& rkQ, bool shortestPath)
    {
        ...
        if (std::fabs(fCos) < 1 - msEpsilon)
        {
            // Standard case (slerp)
            float fSin = std::sqrt(1 - fCos * fCos);
            Radian fAngle(std::atan2(fSin, fCos));
            float fInvSin = 1.0f / fSin;
            float fCoeff0 = std::sin((1.0f - fT) * fAngle.valueRadians()) * fInvSin;
            float fCoeff1 = std::sin(fT * fAngle.valueRadians()) * fInvSin;
    ...
    {
        float fCos = rkP.Dot(rkQ);
        Radian fAngle ( std::acos(fCos) );

        if ( std::fabs(fAngle.valueRadians()) < msEpsilon )
            return rkP;
...
//            float fTx  = 2.0*x;
        ...
        {
            return Radian(atan2f(2.0f * (x*y + w*z), w*w + x*x - y*y - z*z));
        }
    }
    //-----------------------------------------------------------------------
...
//            float fTy  = 2.0f*y;
    ...
    Radian Quaternion::getYaw(bool reprojectAxis) const
    {
        ...
        {
            // internal version
            return Radian(asinf(-2.0f * (x*z - w*y)));
        }
    }
    //-----------------------------------------------------------------------
